...
public class EstoqueService {
	...
	public Estoque buscarPorProdutoId(Long produtoId) {
		return estoqueRepository.findByProdutoId(produtoId).orElseThrow(
				() -> new ResourceNotFoundException("Estoque não encontrado para o produto ID: " + produtoId));
	}
	...
	public void executarAjuste(Long produtoId, Integer quantidade, TipoMovimentacao tipo, Long usuarioId) {
		...
		Estoque estoque = estoqueRepository.findByProdutoIdForUpdate(produtoId).orElseThrow(
				() -> new ResourceNotFoundException("Estoque não encontrado para o produto ID: " + produtoId));

		if (tipo == TipoMovimentacao.SAIDA) {
			if (estoque.getQuantidadeDisponivel() < quantidade) {
				throw new EstoqueInsuficienteException(
						"Estoque insuficiente para a saída. Disponível: " + estoque.getQuantidadeDisponivel());
			}
	...
	public void reservarEstoqueParaCarrinho(Long produtoId, int quantidade, Long carrinhoId) {
		...
		Estoque estoque = estoqueRepository.findByProdutoIdForUpdate(produtoId).orElseThrow(
				() -> new EstoqueInsuficienteException("Produto não possui registro de estoque: " + produtoId));

		if (estoque.getQuantidadeDisponivel() < quantidade) {
			throw new EstoqueInsuficienteException(
					"Estoque insuficiente. Disponível: " + estoque.getQuantidadeDisponivel());
		}
	...
	public void confirmarSaidaDeCarrinho(Long produtoId, int quantidade, Long carrinhoId) {
		...
		if (estoque.getQuantidadeReservada() < quantidade) {
			throw new IllegalStateException(
					"Tentativa de confirmar saída de mais estoque do que o reservado. Reservado: "
							+ estoque.getQuantidadeReservada() + ", Solicitado: " + quantidade);
		}
	...
	private void registrarMovimentacao(Estoque estoque, Integer quantidade, TipoMovimentacao tipo,
			OrigemMovimentacao origem, Long referenciaId) {
