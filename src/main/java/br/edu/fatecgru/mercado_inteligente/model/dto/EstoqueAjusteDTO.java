...
public record EstoqueAjusteDTO(
    @Schema(description = "ID do produto a ser ajustado", example = "1")
    @NotNull(message = "O ID do produto é obrigatório")
    Long produtoId,
    
    @Schema(description = "Quantidade a ser adicionada ou removida", example = "10")
    @NotNull(message = "A quantidade é obrigatória")
    @Positive(message = "A quantidade deve ser maior que zero")
    Integer quantidade,
    
    @Schema(description = "Tipo do ajuste: ENTRADA (soma ao disponível) ou SAIDA (subtrai do disponível)", example = "ENTRADA")
